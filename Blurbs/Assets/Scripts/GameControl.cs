...
public class GameControl : MonoBehaviour
{
	...
	public TMP_Text _text; // вывод текстовой информации
	...
	void Load()
	{
		...
		for(int y = 0; y < 4; y++)
		{
			for(int x = 0; x < 4; x++)
			{
				...
				if(j >= 0)
				{
					grid[x,y] = Instantiate(_puzzle[j], position[x,y], Quaternion.Euler(0,0,180)) as GameObject;
					grid[x,y].name = "ID-"+i;
	...
	void CreatePuzzle()
	{
		...
		for(int y = 0; y < 4; y++)
		{
			for(int x = 0; x < 4; x++)
			{
				...
				if(grid[x,y] == null)
				{
					grid[x,y] = Instantiate(puzzleRandom[i], position[x,y], Quaternion.Euler(0,0,180)) as GameObject;
					grid[x,y].name = "ID-"+i;
					grid[x,y].transform.parent = transform;
					i++;
				}
	...
	void RandomPuzzle()
	{
		...
		while(c < _puzzle.Length)
		{
			...
			if(tmp[r] == 1)
			{ 
				puzzleRandom[c] = Instantiate(_puzzle[r], new Vector3(0, 10, 0), Quaternion.Euler(0,0,180)) as GameObject;
				tmp[r] = 0;
				c++;
			}
		}
